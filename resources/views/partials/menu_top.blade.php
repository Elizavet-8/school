<div style="margin-left:0; margin-right:0" class="row account-block">
    <button onclick="location.href='{{ route('course.show.user', $theme->chapter->course['id']) }}'"
            class="btn-active account__btn">
        <svg display="none">
    ...
    <div class="account-elem">
        ...
        <p class="exp-prg">
            {{ $theme->chapter->themes->count() }} тем <br>({{ $theme->chapter->complete_count(Auth::user()->id) }}
            выполнено)
        </p>
